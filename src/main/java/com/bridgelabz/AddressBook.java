...
public class AddressBook {
    ...
    public void displayMenu () {
        ...
        System.out.println("7. Write contacts to csv file ");
        System.out.println("8. Write contacts to json file ");
        System.out.println("9. Exit this address book");
    }
    ...
    private void writeContactsToCSVFile() {
        File file = new File("/users/nusrat/Desktop/ContactBook.csv");
        try {
            int i = 0;
            ...
            allContacts.add(new String[] { "First Name", "Last Name", "House Id" , "City" , "State" , "Zip" ,"Phone Number" , "Email" });
            if(contactList.size() == 0)
                System.out.println("Contact list is empty!");
            else
                while (i < contactList.size()) {
                    allContacts.add(new String[] {contactList.get(i).getFirstName(),contactList.get(i).getLastName(),contactList.get(i).getAddress(),contactList.get(i).getCity(),contactList.get(i).getState(),contactList.get(i).getZip(),contactList.get(i).getPhoneNum(),contactList.get(i).getEmail()});
                    i++;
                }
    ...
    private void writeContactsToJSONFile() {
        File file = new File("/users/nusrat/Desktop/ContactBook.json");
        try {
            int i = 0;
            Gson gson = new Gson();
            FileWriter outputFile = new FileWriter(file);
